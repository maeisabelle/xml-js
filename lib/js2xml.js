...
var currentElement, currentElementName, currentElementWithClosingTag;
...
function writeElementCompact(element, options, depth, currentElementIndex) {
  ...
  if(currentElementIndex == 0) {
    ...
    currentElementWithClosingTag = withClosingTag;
    if (withClosingTag) {
      xml.push('>');
    } else {
      xml.push('/>');
    ...
    if (element.hasOwnProperty(key)) {
      ...
      for (i = 0; i < nodes.length; ++i) {
        ...
        case options.declarationKey: 
          var output = writeDeclaration(nodes[i], options, depth) + getNewLineIfWithSpaces(options);
          xml.push(output);
          break;
        case options.instructionKey: 
          var output = (options.indentInstruction ? writeIndentation(options, depth, firstLine) : '') + 
            writeInstruction(nodes[i], options, depth) + getNewLineIfWithSpaces(options);
          xml.push(output); 
          break;
        ...
        case options.cdataKey: 
          ...
          break;
        case options.doctypeKey: 
          var output = writeIndentation(options, depth, false) + writeDoctype(nodes[i], options);
          xml.push(output); 
          break;
        case options.commentKey: 
          var output = writeIndentation(options, depth, firstLine) + writeComment(nodes[i], options) + getNewLineIfWithSpaces(options);
          xml.push(output); 
          break;
        default: 
...
function getNewLineIfWithSpaces(options) {
